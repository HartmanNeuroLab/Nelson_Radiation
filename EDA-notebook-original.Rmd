...
Use Excel to generate a .csv file with "tidy" data (each row = 1 case / subject, 1st row is column names). Import .CSV file into R "dataframe" called "watermazedata". The show the "watermazedata" dataframe (header + 1st 8 data rows) to check it out

```{r}
watermazedata <- read.csv(file="water maze all.csv", header=TRUE, sep=",")
watermazedata
```
...
# Test for homogeneity of variance among groups using the leveneTest() function from the car package (default uses median)... to use mean instead of median: leveneTest(watermazedata$Duration.Spatial, watermazedata$Treatment, center = mean)
...
Visually check each variable's data for normality averaged across all groups. This info is probably not all that interestng until broken down by group.

```{r}
# Histograms with overlaid normal curve and Quantile–quantile plots (ADD SCATTERPLOTS)
...
Now, visually check each variable data for normality broken down by group - histograms and QQ plots and box plots for each Treatment group.
...
For non-normally-distributed data:
 ...
 - Transform the data: log, square root, or reciprocal transformations can correct for positive skew and/or unequal variance. If data are negatively skewed, you need derive a reciprocal score (reverse the scores by subtracting each score from the highest score obtained)
  -- Make new transformed DVs using newVariable <- function(oldVariable)
    ...
    --- Reciprocal: watermazedata$Duration.Spatial.Reciprocal <- 1/(watermazedata$Duration.Spatial +1)
            +1 needed to avoid trying to divide by zero
