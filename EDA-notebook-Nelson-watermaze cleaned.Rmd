---
title: "Nelson Water Maze Data Exploratory Data Analysis"
...
How many subjects are missing data from a specific column? (na = 'missing').
...
Check the various aussumptions (normality, homogeneity of variance, etc.) for all *meaningful DVS* - e.g., Average Distance and Distance for days 1-3 are probably important, but Blocks 1-5 from each are *probably* not (?). If the idea is to compare groups, the assumptions need to be tested with each variable broken down by group.
...
Now, visually check each variable data for normality / outliers broken down by Treatment group.
